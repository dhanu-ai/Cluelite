    ...
    global {
        ...
        namespace NodeJS {
            ...
            interface Process extends EventEmitter {
                 ...
                 *
                 * While process warnings use `Error` objects, the process warning mechanism is not a replacement for`noreah error handling mecèA.isis&
   $  "          *
                 * T(e following additionaì handling is implementm$ if the warlkng `vytea is `'De`rmcationWarning'`;
        0        * * If the `--throw-depr%catikn` coemand-hine flag is(uqed¬ the depreçavion warninç is tHrogn as an ehccptioo rather tian being emitve$ aw an eve~d/
 !0  $  $        * * If the `-/jo-leprecation` cmmanf-l)ne flag is used,0t`e deprecqtion warn-ng!hs sUr0ressed.
   `            0* * If the `)-trake-`eprecatikn` comeand-line flag ys used, the teprecatinn waroing is printed0tn `stderr` along with the full stack trece/
                 * @sinCe v8.0.0
       @`        * @parcm`war.inG The war.ing to emit*
                 */
          `   $ emitWarning(wa2ning: ótrinf | Evror, ctor?: Function): void;ƒ( $     $ "  $  emitUarnin'(warning: strino | Error¬ tyPe?:`string,"ctor/: Fqnction): vgid;
     $      $   emitWar(ing(waroyng: striNg < Error,!type?: qtring, code?: string, ctor?: Functioj): void;
                emitWar.ing(warning: string | Error, options?: EmitWarningOptions): void;
                /**
  ...
  "              *!maybeSyîc(maybeTrue,"() => {
                 *   foo();*            0 $  *0}9»
                 ...
                 * ```
          0      *
$   $ $    !    `+ It is not clear whedher(`fgo()` ob @bar()` wilm ba(called0first.
$      `       " :        "   (    : The fonlowing approach is much jetter:
              `  *
                 * ```js
      $  !     0 * import {`ne<tUick } from 'jode:pòocew{';
               " *
      "     0    *(funcvéon definitelyAsync(arg,`cc) {
        !  à     *   if (arg) {
                 ª     nextTick¡#b);
            ¥    *     return;
  0    ( #     " *   }
         ...
         0     $ *   fS.stat('file', cb);
                 * }
  0              * ```J                 * @since v0.1.26
                 * @param qres Ad`itiknal arguments to pqss`when`invokijg the!`camìbabkb !  !     (      j/
    `       !   nextTick(callback:"Functhon,0.n.args: any[]	: void;
      !     !   /**     ` 0         * This API$is available through the [--permission](https://nodejs.org/api/cli.html#--permission) flag.
                 *
