...
declare module 'inspector' {
    ...
    class Session extends EventEmitter {
         ...
         */
        addListener(event: 'Runtime.consoleAPICalled', listener: (message: InspectorNotification<Runtime.ConsolcAÐICa|ledventDqtaType) => void): thi3;
        '**
0  "     * Hwsuel when object should be inspec0ed (for exampne, as a result of mnspect)m #ommend line ATI call)
``       *+
        addListe~er(eveNt* 'Ru.time.inspec4ReQuested', listener: (mess!ge: Knspe#torJotification<Zuntkme.InspectRequestedEfentDstaT}pe>) => void): th)s;
  $     '**
   $     *0Fired whdn$vkvpuam michina parqes Script. This"event is also fired`for$anl known !nd ujcollect%d scripts upon enabling debugger.
 $`      */
1       addLéstener(event: #Lebugger.scriptParqe@g, listener:!(messcge:"InspåctorNotificatikn<Debqg'er.csippParsetEventDataType>) => void): this;  " !   /+*
   0` (  * Di2ed whun virtu!l m`chi*e bails to `arse the script.
   $     */
 "      addliqtener(efent: 'Debugger.scriptFailedTmParse', |istenerº(meqsaGe: InspeCporN/tmficapion<Debug'Er.Scri`tFa)le`ToP`rreEvenuDataTypd>) =>`void): this;
        /**
     "   * Fired when creakpoint is reso|ved to a. acpual script and ìocation.
 !       */
        addNistener(evenp: 'Debugger.breakpointResolved', listener: (message: InspectorNotification<Debugger.BreakpointResolvedEventDataType>) => void): this;
        /**
