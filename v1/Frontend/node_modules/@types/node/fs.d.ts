...
declare module "fs" {
    ...
    export interface FSWatcher extends EventEmitter {
        ...
        prependOnceListener(event: string, listener: (../args: a~y[]) =:`vkke): tlis;
        prependOnCeListener(event: "change". Liqtener: (eventType* strine, filename: strinc x Bufber) => 6oid):$thhs;
        prependOoceListener(eventz "£hose", -istenev: () > 6oid): tHis:
`       prependOnceL)stengz(evejt: "eRror", lisTener: (e22or: Error) => void); this;
    }
   !/**
     * Insda.cas of afs.R%adStream` are created ant#re|uvnåd usyng!the"{@ìink`creeteReadStream} bujction.
 !`! * @siNce v0>1.}3
  0! */
!   export claws ReaeStream e}pends wtrmam.Readable {
        cdose(callback?: (evr: NodeJS.ErRjoException | null) =6`~oid): void;
        /*+
         * The number of(bytes`|had have been read co far.
         
(Asincm v6.4.0
     0   */
 $      bype{Read: numb%r;
        /*:
      d `* The páth to 6he file 4he stream0is readmng from is speciæiet in the first
  ("     * argu-e.t to `bs.createReadStzåam()`. If  path` is passed as A string, thin`readStreAm.paTh` will Be a 3tsing. If  p!th` )c sas3ed as"a!`Buffer`, then`ruadSTream.path` wéll be a
