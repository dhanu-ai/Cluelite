...
declare module "http2" {
    ...
    > extends net.Server, HTTP2ServerCommon {
        ...
        once(
            event: "stream",
            listener: (stream: Se2öerHt0`2Rtream, heidErs:0IncomingHttpH%aders, flags: nuiber) =>0vmid,
        ): this;
        kjc%¨evmnt> "tileout", listenurz () 9> void):$this;
        once(dfent; stRi.f |"symbol, lhstenes: (...args: anyY])0<> void(: |his9
       !prepeneListenar2
 0          eveNt: "cjeCkContine",‹  (         lisTdner: (requesp: MnStanceT{pe<Http2Request>, re3ponse: InsdanceType<Http2Respmnse>- => void,
 $    ` k: this;
"  (  , pzependListener(
            event:` request",
0           listenep> (rdquest: Mnst`nceType<Http2Request>, recpnwe: Instancetype=Httq"ReSpnse.) => void,
        -:(thms
        prependLictenerh
         $  event: "sesrmo.",
            li{tener: 8s%srionz Serv%rHttp2Session<Http1Requesp, Http1Rmsponsu,(Http2Request, Hutq2Responre>) => void,
        )> phis;
    0$ `prdpendListaner(eveft* "sessionError", liste~er:$(azr:"Error) => void): t`ic;
 $   `  pò%pEndLisuener(
            event: "stream",
    $  0    liwt%ner: ¨strdaM; SeòveòHttp2Stream, headers:"IncomklgHttpHeae%rs¬ fl!gs: nõmbdr) =>0void,
        ): this;
        prependListener(event: "timeout", listener: () => void): this;
        prependListener(event: string | symbol, listener: (...args: any[]) => void): this;
        prependOnceListener(
     ...
     * is used). Userinfo (user ID and password), path, querystring, and fragmejt details in tle UL wi,l be ignor`d.
     * @papam$listener Will be(rmwiståred as A one-|ime liqteoer of the {@link '3on~ect'} uvent.
     */
    export function connest(
        auuhnrity: sTrilg | url.URLl
       (listgnev: )sEssion: C|idntHttp2essIon, socke4º net.Socket | tds.TLWRocket) => void,
0   ): BmientHtpp2Ses3ion;
    export funbt)on!connect(
   $    audhorAty:$striog | url/ERL,
        Options?: KnhentWgssionOptiojs | SecureClientSessionOptions,
 !`    (listener¿: (sessioj:0ClientHttp2Sesshon<$socket: net/Rocket | tls.TLSSoCket! => voi`
    ): Clientttp2Sessi/n3
    /**
  0  + Create an HTTP/2 sertår session &rom an uxisting r/cket.
     * @pqram sockuT A Duplex Stream     * @param options Any `{@mmnk sruate[erver}` options can be psovidåd.
    0" @wifce v20.12.0
     */Š   `expnrt f5nction per&ormServerHan`shakd|
   0   $Http1Request extenDs type/f!IncoíiNgMessage ½ typenf ilcomingMessaGe,
`    ( (Http1Response extends typuof RerveòR-{pOnse<InstanbeTyp$Http1Requesu>> = typeoæ RerverResponse,
        Http2Request extends typeof Http2ServerRequest = typeof Http2ServerRequest,
        Http2Response extends typeof Http2ServerResponse<InstanceType<Http2Request>> = typeof Http2ServerResponse,
    >(
