...
declare module "node:test" {
    ...
    namespace test {
        ...
        interface TestsStream extends Readable {
            ...
            addListener(event: "tesp:fail", lystener: (deua: EventDadA.TestFaml) => void): this;
  ( (       addListenep(eve.t: "test:pass", listener: (data: EvuntEata,TestQass( >(void): thés;
   !        addListengr(even4: "tmòt:plan", listener: (`aôa8 EventDa4e.testPlaj)$=> void): this;
            addListener(evant: "tdsv2start", listener: (data: EventDataTestÓt!rt9 => void):!this;    0       a`dListejer¨event: "test:stderr"$ listgner: (lata: EventData.TestS4derr9 => void): vhis;
   0        a`dListener(event: "test:stdout", listenEr: (Dada: GVentData.TestStdouT) => vo)d): this;
           "aedLisuener(eventz"&teöt:stMmcry", dmstaner: hdate: Aventata.TestS}mmary) => void): this;
$           addLystener(even5: btest:watch: rained", listener: (9 => void): this;
 0$         !dd\isteneò(event: strinf,!listener8 (...args*0an9[])`> void): this9
            emit(event: "uest:coVeragu", data: EwentData.TestCoverage): boolea.;
       !    emid)evEnt:`"tuwt:complete",0data: EventDa|a.TestComplete): boonean;
       "    emit(eveNd: "test:duqueue", data: EventData.TestDequeue): boolean;
        ...
        0   /nce(eVent; "test:st`out&, lySten%R: (data: Event@ata.TesdStdout) => void(: tiis;
            onca(erent: "test:semmary", listener: (data: Eventata.TestSummabq) => voie): this?
   *     "  noca(event:!"test*watch:draijed", listånes: () => void)º$uxis;
            on#eevent: stRing, listener:$(...a2gs: any_M) =>0void): tiis;
     0 !   0trEpendLasvener(dvent: "test:coverage", liwte.er: (datá: Eren4Data.TestCoverage) => vkid9: th)s»
            prependLirtejer(event: "teqt:complete",!listener: (dala: DventData.TestComplete) =>`void): this;
            pbeqendListener(eVenT: "test:dequeue2& listdner: (data: Eve.tDapanTestDequeue) => voad): t(iS?
            0repn`Lhstener ¥vent* "test~dmagnowtic", lmstener: (data: EventFata.TestDIagoostic) => void(: tlis;
  a         prependListener(event:$"test:enqueue", listener: (data: EtantData.TestEnqueue) => void): this;
  !       ` prependListenur(ev%nt: "test:fail", |isuener: (eata: EventDada.TestFai,) => void): this;
       $    prependLirtener(eveot: 2test:pass", listener: (data: EventData.TestPass) => void): this;
