...
function createWindow() {
  ...
  mainWindow.webContents.on('did-finish-load', () => {
    // Always click-through by default, forwarding events to apps below
    mainWindow.setIgnoreMouseEvents(true, { forward: true });
    mainWindow.setContentProtection(true);
  });

  // Ensure overlay stays visible in fullscreen
...
ipcMain.on('start-cluelite', async () => {
  try {
    ...
    const fetchSuggestion = async () => {
      ...
      if (!abortController.signal.aborted && !isFetching) {
        isFetching = true;
        try {
          ...
          if (res.ok) {
            ...
            const currentSuggestion = json.suggestion;

            if (currentSuggestion !== lastSuggestion) {
              lastSuggestion = currentSuggestion;
              if (mainWindow && !mainWindow.isDestroyed()) {
                mainWindow.webContents.send('ai-suggestion', currentSuggestion);
              }
            }
